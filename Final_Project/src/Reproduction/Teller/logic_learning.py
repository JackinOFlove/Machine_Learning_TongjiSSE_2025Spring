...
class Logic_Learning_Model():
    def __init__(self, head_predicate_idx = [4]):
        self.predicate_set= [0, 1, 2, 3] # 所有有意义的谓词集合
        self.predicate_notation = ['A','B', 'C', 'D']
        self.instant_pred_set = []
        self.survival_pred_set = []
        self.body_pred_set = []
        self.head_predicate_set = head_predicate_idx.copy()  # 头谓词的索引集合
    ...
    def intensity(self, cur_time, head_predicate_idx, dataset, sample_ID):
        ...
        if self.use_exp_kernel:
            ...
            intensity = torch.exp(intensity) + 1e-1
        else:
            intensity = base + torch.sum(intensity)
            intensity = torch.nn.functional.relu(intensity) + 1e-1
    ...
    def get_feature(self, cur_time, head_predicate_idx, history, template):
        ...
        # 每当transition_item_dic中的任何项目不为空时计算特征
    ...
    def print_rule(self):
        ...
        print("logic_template:{}".format(self.logic_template))
    ...
    def RAFS(self, head_predicate_idx, training_dataset, testing_dataset, tag, init_params=True):
        ...
        with Timer("initial optimize") as t:
            ...
            nll, mae, rmse = self.evaluate(head_predicate_idx=head_predicate_idx, dataset=testing_dataset)
            print("Test NLL", nll.item(), "MAE: ", mae.item(), "RMSE: ", rmse.item())
    ...
    def evaluate(self, head_predicate_idx, dataset):
        nll, mae, mse = 0.0, 0.0, 0.0
        count = 0
        for sample_ID in dataset:
            gt_times = dataset[sample_ID][head_predicate_idx]["time"]
            if gt_times[0] == 0:
                gt_times = gt_times[1:]
            if gt_times == []:
                count += 1
                continue
            pred_times, preceding_times, all_times = [], [], []
            for predicate_ID in dataset[sample_ID]:
                all_times.extend(dataset[sample_ID][predicate_ID]['time'])
            all_times = sorted(set(all_times))
            for current_time in gt_times:
                preceding_time = 0
                low, high = 0, len(all_times) - 1
                while low <= high:
                    mid = (low + high) // 2
                    if all_times[mid] < current_time:
                        preceding_time = all_times[mid]
                        low = mid + 1
                    else:
                        high = mid - 1
                preceding_times.append(preceding_time)
            for prec_time in preceding_times:
                predict_time = 1.0 / self.intensity(prec_time, head_predicate_idx, dataset, sample_ID)
                pred_times.append(prec_time + predict_time)
            single_nll = 0.0
            for t in gt_times:
                single_nll += -torch.log(self.intensity(t, head_predicate_idx, dataset, sample_ID))
            num_points = 20
            T = max(all_times)
            times = torch.linspace(0, T, num_points).double()
            intensity_values = torch.tensor([self.intensity(t.detach().numpy(), head_predicate_idx, dataset, sample_ID).item() for t in times])
            single_nll += torch.trapz(intensity_values, times)
            single_mse, single_mae = 0.0, 0.0
            for t, that in zip(gt_times, pred_times):
                single_mse += (t-that)**2 / len(gt_times)
                single_mae += abs(t-that) / len(gt_times)
            mae += single_mae / len(dataset)
            mse += single_mse / len(dataset)
            nll += single_nll / len(dataset)
        return nll, mae, mse**0.5
